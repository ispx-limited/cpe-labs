...
- `example-sagemcom-fast5599/`: TR-181 WiFi 7 fibre gateway with five switch ports, a 10 Gbit/s uplink enumerated last in `Device.Ethernet.Interface.`, an SFP cage that also reports `Upstream=true` while dormant, three radios including 6 GHz across nine access points, local accounts under `Device.UserAccounts.User.` rather than `Device.Users.User.`, and a mesh kept under `Device.WiFi.MultiAP.X_SAGEMCOM_MultiAPConfig` with no APDevice or Data Elements table. Converted from a real unit's data model dump, so the tree is the device's rather than an idealised one
